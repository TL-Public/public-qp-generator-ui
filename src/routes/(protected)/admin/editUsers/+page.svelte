<script>
  ...
  onMount(() => {
    ...
    if (!userId) {
      goto('/admin?error=' + encodeURIComponent('No user ID provided'));
      return;
    }
  });
  ...
  async function loadUser(id) {
    ...
    try {
      ...
      const foundUser = response.data.users.find(u => u.id.toString() === id.toString());
  ...
  function handleUserUpdated(event) {
    ...
    const successMessage = message || `User "${updatedUser.username}" updated successfully!`;
    goto('/admin?userUpdated=true&message=' + encodeURIComponent(successMessage));
  }
  ...
  <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="mb-8">
      ...
      <p class="mt-2 text-gray-600">
        {#if user}
          Update user details and permissions for <span class="font-medium">{user.username}</span>
        {:else if loading}
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
      ...
      {:else}
        ...
        <div class="p-8 text-center">
          ...
          <p class="text-sm text-gray-600 mb-6">The requested user could not be found.</p>
          <button
            type="button"
            on:click={handleClose}
            class="inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
