...
{:else if data.length === 0}
  <div class="text-center py-12">
    ...
    <p class="mt-1 text-sm text-gray-500">No papers match your search criteria.</p>
  </div>
{:else}
                ...
                >
                  ...
                  {:else if header.key === 'exam_code'}
                    <button
                      type="button"
                      class="text-blue-600 hover:text-blue-800 focus:outline-none focus:underline text-left font-medium transition-colors duration-150"
                      on:click={() => dispatch('paperSummary', paper[header.key])}
                    >
                      {paper[header.key]}  
                    </button>
                  ...
                  {:else if header.key === 'status'}
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {paper[header.key] === 'closed' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}">
                      {paper[header.key]}
                    </span>
                  {:else}
