<script>
  ...
  async function getSelectedChaptersAndTopics() {
    ...
    console.log("filtered chapters", chapterSelections);
    console.log("filtered topics", topicSelections);
...
function handleConfirmAndReview(event){
  console.log('Nestedcontent table : forwarding confirmed', event.detail) ; 
  ...
  async function loadQuestions(chapters, topics) {
    ...
    try {
      ...
      if (chapters && chapters.length > 0) {
        ...
        apiCalls.push(
          api.questions.getByGroupCodes({ type: "chapter", codes: chapterCodes })
        );
      }
      ...
      console.log('Cleaned questions:', allQuestions);
     return allQuestions ; 
    } catch (err) {
      error = err.message;
  ...
  async function loadChapters() {
    ...
    try {
      ...
      console.log('the data fetched for chapters and topics', response.data.data);
  ...
  async function handleFetchQuestions(event) {
    ...
    if (allQuestions && allQuestions.length > 0) {
      ...
      console.log('Mapped fetchedQuestions:', fetchedQuestions); // Add this log
      ...
      console.log('No questions found or allQuestions is empty');
      fetchedQuestions = [];
    }
  }
  ...
  function handleQuestionRemoved(event) {
    ...
    console.log(`Question ${id} ${removed ? "removed" : "restored"}`);
  ...
  // Forward allocation confirmation event
   function handleAllocationConfirmed(event) {
    console.log('NestedContentTable received allocation confirmation:', event.detail);
    // Forward the event detail to the parent (page.svelte)
    console.log('Nested content : forwarding allocation event', event.detail);
  ...
  function hasSelectedTopicsInChapter(chapter) {
    ...
    return chapter.topics.some(topic => 
      ...
      (topic.subtopics && topic.subtopics.some(subtopic => isSubtopicSelected(subtopic.code)))
    );
  }
  ...
  export function resetToContentSelection() { 
    console.log('Resetting to content selection');
    showQuestions= false ; 
    activeTab = 'questions' ; 
    fetchedQuestions = [] ; 
              ...
              <tr class="hover:bg-gray-50" data-item-code={chapter.code}>
                <td class="px-6 py-4">
                  <div class="flex items-center space-x-2">
                    ...
                    >
                      <svg
                        class="h-4 w-4 transition-transform duration-200 {expandedChapters.has(chapter.code)
                          ? 'transform rotate-90'
                          : ''}"
                        viewBox="0 0 20 20"
                        fill="currentColor"
                      ...
                      <div class="flex items-center space-x-2">
                        ...
                        >
                          {#if topic.subtopics && topic.subtopics.length > 0}
                            <svg
                              class="h-4 w-4 transition-transform duration-200 {expandedTopics.has(topic.code)
                                ? 'transform rotate-90'
                                : ''}"
                              viewBox="0 0 20 20"
                              fill="currentColor"
        ...
        <div class="border-b border-gray-200">
          <nav class="flex space-x-0">
            <Tab 
              ...
              on:click={() => browser && goto('#selected-content', { replaceState: true, noScroll: true })}
