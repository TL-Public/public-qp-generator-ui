<script>
  ...
  function handleClearAll() {
    if (confirm('Are you sure you want to clear all removed questions? This action cannot be undone.')) {
...
<div class="bg-white">
  ...
  <div class="flex items-center justify-between mb-6">
    <div>
      <h3 class="text-lg font-medium text-gray-900">Removed Questions</h3>
      <p class="text-sm text-gray-500">
        {removedQuestions.length} question{removedQuestions.length !== 1 ? 's' : ''} removed from the paper
      </p>
    </div>
  ...
  {#if removedQuestions.length > 0}
    ...
    <div class="mt-4 p-4 bg-gray-50 rounded-lg">
      <div class="flex items-center justify-between text-sm text-gray-600">
        <div>
          Total removed questions: <span class="font-medium">{removedQuestions.length}</span>
        </div>
      </div>
    </div>
  {:else}
    ...
    <div class="text-center py-12">
      ...
      <h3 class="mt-2 text-sm font-medium text-gray-900">No removed questions</h3>
      <p class="mt-1 text-sm text-gray-500">
        Questions that you remove from the paper will appear here.
      </p>
    </div>
  {/if}
</div>