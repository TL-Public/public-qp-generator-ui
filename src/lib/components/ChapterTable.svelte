  ...
  $: filteredItems = (() => {
    ...
    const filtered = items.filter(
      (item) =>
        ...
        (item.parentTopic &&
          item.parentTopic.toLowerCase().includes(searchQuery.toLowerCase())),
    );
    ...
    return filtered.sort((a, b) => {
      ...
      switch (sortField) {
        ...
        case "parentChapter":
          return (
            (a.parentChapter || "").localeCompare(b.parentChapter || "") *
            modifier
          );
        case "parentTopic":
          return (
            (a.parentTopic || "").localeCompare(b.parentTopic || "") * modifier
          );
        default:
        ...
        <tr class="bg-gray-50">
          ...
          {#if selectionType !== "chapter"}
            <th
              ...
              on:click={() =>
                handleSort(
                  selectionType === "topic" ? "parentChapter" : "parentTopic",
                )}
            >
              <div class="flex items-center space-x-1">
          ...
          >
            <div class="flex items-center space-x-1">
              <span>Questions</span>
              <span class="text-gray-400"
                >{getSortIndicator("questionCount")}</span
              >
            </div>
          </th>
          <th class="w-24 px-4 py-3 text-sm font-medium text-left text-gray-700"
            >Actions</th
          >
        </tr>
      </thead>
    ...
    <div class="flex items-center justify-between border-t pt-4">
      <div class="flex items-center space-x-2">
        ...
        {#each Array(totalPages) as _, i}
          <button
            class="px-3 py-1 border rounded-md {currentPage === i + 1
        ...
        <button
          class="px-3 py-1 border rounded-md {currentPage === totalPages
            ? 'text-gray-400'
            : 'hover:bg-gray-50'}"
