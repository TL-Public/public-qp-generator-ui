<script>
  ...
  $: availabilityPercentage = totalRequired > 0 ? Math.round((totalAvailable / totalRequired) * 100) : 0;

  // Get only selected items for detailed breakdown
  $: selectedItemsWithQuestions = selectedItems.filter(item => item.isSelected && item.questionsToAdd > 0);
...
{#if showModal}
  <!-- Modal Backdrop -->
  <div 
    class="fixed inset-0 backdrop-blur-xs z-50 flex items-center justify-center p-4"
  ...
  >
    ...
    <div class="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-hidden">
      ...
      <div class="px-6 py-4 overflow-y-auto max-h-[60vh]">
        ...
        <div class="mb-6">
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
            <div class="bg-blue-50 border border-blue-200 rounded-lg p-3 text-center">
              <div class="text-2xl font-bold text-blue-600">{totalRequired}</div>
              <div class="text-sm text-blue-800">Required</div>
            </div>
            <div class="bg-green-50 border border-green-200 rounded-lg p-3 text-center">
              <div class="text-2xl font-bold text-green-600">{totalAllocated}</div>
              <div class="text-sm text-green-800">Allocated</div>
            </div>
            <div class="bg-purple-50 border border-purple-200 rounded-lg p-3 text-center">
              <div class="text-2xl font-bold text-purple-600">{totalAvailable}</div>
              <div class="text-sm text-purple-800">Available</div>
            </div>
            {#if !allocateWithAI}
            <div class="bg-gray-50 border border-gray-200 rounded-lg p-3 text-center">
              <div class="text-2xl font-bold {remaining === 0 ? 'text-green-600' : remaining > 0 ? 'text-amber-600' : 'text-red-600'}">
                {remaining}
              </div>
          ...
          <div class="mt-4">
            {#if remaining === 0}
              <div class="bg-green-50 border border-green-200 rounded-md p-3 text-sm text-green-800">
                ✅ Perfect allocation! All {totalRequired} questions are properly allocated.
              </div>
            {:else if remaining > 0}
              <div class="bg-amber-50 border border-amber-200 rounded-md p-3 text-sm text-amber-800">
                ⚠️ {remaining} questions remaining. These will be auto-allocated from available content.
              </div>
            {:else}
              <div class="bg-red-50 border border-red-200 rounded-md p-3 text-sm text-red-800">
                ❌ Over-allocated by {Math.abs(remaining)} questions. Please adjust your allocation.
              </div>
            {/if}
          </div>
          {/if}
          ...
          <div class="bg-gray-50 border border-gray-200 rounded-lg p-4">
            <div class="flex items-center space-x-4 text-sm">
              <div>
                ...
                <span class="ml-1 px-2 py-1 bg-blue-100 text-blue-800 rounded-md text-xs font-medium">
                  {allocationType}
                </span>
              </div>
              <div>
                <span class="font-medium text-gray-700">Level:</span>
                <span class="ml-1 px-2 py-1 bg-green-100 text-green-800 rounded-md text-xs font-medium">
                  {allocationLevel}
                </span>
              </div>
            </div>
        ...
        <div class="mb-6">
          <h3 class="text-lg font-medium text-gray-900 mb-4">Detailed Allocation Breakdown</h3>
          
          {#if selectedItemsWithQuestions.length === 0}
            <div class="text-center text-gray-500 py-4">
              <div class="text-sm">No items with question allocations found.</div>
            </div>
          {:else}
            <div class="space-y-3">
                  ...
                  <div class="flex-1">
                    ...
                    <div class="flex items-center space-x-2 mt-1">
                      <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium
                        {item.type === 'chapter' ? 'bg-blue-100 text-blue-800' : 
                          item.type === 'topic' ? 'bg-green-100 text-green-800' : 
                          'bg-purple-100 text-purple-800'}">
                        {item.type}
                      </span>
                      <span class="text-xs text-gray-500">Code: {item.code}</span>
                    </div>
                  </div>
                  <div class="text-right">
                    <div class="text-lg font-bold text-blue-600">{item.questionsToAdd}</div>
        ...
        {#if autoAllocations.length > 0}
          <div class="mb-6">
            <h3 class="text-lg font-medium text-gray-900 mb-4">Auto Allocations</h3>
            <div class="space-y-2">
              {#each autoAllocations as allocation}
                <div class="p-3 bg-blue-50 border border-blue-200 rounded-lg">
                  <div class="text-sm text-blue-800">{allocation.description}</div>
                </div>
              {/each}
            </div>
          </div>
