<script>
  ...
  $: isSelected =
    selectionLogic?.isTopicLogicallySelected?.(topic.code, chapter.code) ||
    false;
  $: isHighlighted =
    highlightedItem?.code === topic.code && highlightedItem?.type === "topic";
  $: selectedSubtopics =
...
<tr
  class="hover:bg-gray-50 transition-colors duration-150 {isHighlighted
  ...
  <td class="px-6 py-3 whitespace-nowrap">
    <div class="flex items-center">
      ...
      {#if topic.subtopics && topic.subtopics.length > 0}
        <button
          type="button"
          class="mr-2 p-1 text-gray-400 hover:text-gray-600 focus:outline-none transition-transform duration-200 {isExpanded
            ? 'rotate-90'
            : ''}"
      ...
      <input
        ...
        class="h-3.5 w-3.5 text-primary focus:ring-blue-500 border-gray-300 rounded mr-3"
  ...
  <td class="px-6 py-3">
    <div class="flex items-center justify-between">
      ...
      {:else if selectedSubtopics.length > 0}
        ...
        >
          {selectedSubtopics.length} subtopic{selectedSubtopics.length !== 1
            ? "s"
            : ""} selected
        </span>
      {:else}
