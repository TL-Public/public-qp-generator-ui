<script>
  ...
  $: totalAllocated = selectedItems.reduce((sum, item) => sum + (item.questionsToAdd || 0), 0);
    ...
    <div class="space-y-4">
      ...
      <div class="flex items-center justify-between border-b pb-3">
        <div class="flex items-center gap-3">
          <h3 class="text-sm font-medium text-gray-900">Question Allocation Summary</h3>
          <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {allocationMode === 'AI' ? 'bg-purple-100 text-purple-800' : 'bg-green-100 text-green-800'}">
            {allocationMode} Mode
          </span>
        </div>
        <div class="text-sm text-gray-600">
          Total Allocated: <span class="font-medium text-gray-900">{totalAllocated}</span> / <span class="font-medium text-gray-900">{totalQuestions}</span>
        </div>
      </div>
                ...
                <tr class="{index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}">
                  ...
                  <td class="px-6 py-4 text-center">
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium 
                      {item.type === 'chapter' ? 'bg-blue-100 text-blue-800' : 
                        item.type === 'topic' ? 'bg-purple-100 text-purple-800' : 
                  ...
                  <td class="px-6 py-4 text-center">
                    <div class="flex items-center justify-center">
                      <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium  text-green-800">
                        {item.questionsToAdd || 0}
                      </span>
                    </div>
                  </td>
      ...
      {:else}
        ...
        <div class="text-center py-8">
          <svg class="mx-auto h-12 w-12 text-gray-400" stroke="currentColor" fill="none" viewBox="0 0 48 48">
            <path d="M8 14v20c0 4.418 7.163 8 16 8 1.381 0 2.721-.087 4-.252M8 14c0 4.418 7.163 8 16 8s16-3.582 16-8M8 14c0-4.418 7.163-8 16-8s16 3.582 16 8m0 0v14m-16-4c1.381 0 2.721-.087 4-.252" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
          </svg>
          <h3 class="mt-2 text-sm font-medium text-gray-900">No allocation data</h3>
          <p class="mt-1 text-sm text-gray-500">
            Question allocation information is not available. Please go back and complete the allocation process.
          </p>
        </div>
      {/if}
    </div>
