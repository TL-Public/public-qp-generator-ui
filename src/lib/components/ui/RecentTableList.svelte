              ...
              <tr class="hover:bg-gray-50 transition-colors duration-150">
                <td class="px-4 py-3 text-sm font-mono text-gray-900">
                  <button
                    ...
                    title="Click to view paper details: exam.exam_code"
                    on:click={() => handlePaperSummary(exam.exam_code)}
                  >
                    {exam.exam_code || 'N/A'}
                  </button>
                </td>
                <td class="px-4 py-3 text-sm text-gray-900">
                  <div class="word-wrap " title={exam.exam_name}>
                    <button
                      type="button" 
                      class="text-blue-600 hover:text-blue-800 focus:outline-none focus:underline text-left font-medium transition-colors duration-150"
                      title="Click to view paper details: exam.exam_name}"
                      on:click={() => handleViewPaper(exam)}
                    >
                    <span class="word-wrap">  {exam.exam_name || 'Untitled Exam'} </span> <br/>
                    <span class="text-xs text-red-600">({exam.total_questions} Questions)</span>
                    
                    </button>
                  </div>
                </td>
                <td class="px-4 py-3 text-sm text-gray-900">
                  <div class="word-wrap " title={exam.status}>
                    <span class="text-xs text-gray-600" title={exam.subject + ' ' + exam.standard + ' ' + exam.medium}>
                      {exam.subject + ' ' + exam.standard + ' ' + exam.medium}
                    </span>
                  </div>
                </td>
                <td class="px-4 py-3 text-sm text-gray-900">
                  <div class="word-wrap " title={exam.status}>
                    <span class="inline-flex word-wrap items-center px-2.5 py-0.5 rounded-full text-xs font-medium {getStatusStyling(exam.status)}">
                ...
                <td class="px-4 py-3 text-sm text-gray-900">
                  <div class="word-wrap " title={exam.status}>
                    <span class="text-xs text-gray-600" title={exam.created_at}>
                ...
                <td class="px-4 py-3 text-sm text-gray-900">
                  {#if exam.status === 'draft'}
                    <button
                      ...
                      on:click={() => handleEdit(exam.status)}
                    >
                      Edit
                   </button>
                   {:else}
