    ...
    <div class="flex justify-between items-center">
      ...
      {#if charCount > 0}
        <button
          type="button"
          on:click={handleSave}
          class="px-4 py-2 text-sm font-medium text-white bg-primary rounded-md hover:bg-primary-hover focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
        >
          Save Remark
        </button>
      {/if}
