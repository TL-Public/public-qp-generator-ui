<script>
 ...
 function generateSubjectCardsFromData(examData) {
  ...
  examData.forEach((exam, index) => {
    ...
    const subjectCode = exam.subject_code || exam.subjectCode || exam.subject_id || exam.code;
    
    // Try different possible property names for medium_code  
    const mediumCode = exam.medium_code || exam.mediumCode || exam.medium_id;
    
    console.log(`Exam ${index}:`, { subject, standard, medium, subjectCode, mediumCode });
  ...
  function generateSubjectDescription(subject) {
    const descriptions = {
      ...
      'Political Science': 'Indian Polity, International Relations & Public Administration'
    };
  ...
  async function loadChaptersTopics() {
    ...
    try {
      ...
      if (response.data && response.data.data && Array.isArray(response.data.data)) {
        ...
        chaptersTopics = [];
        console.warn('Unexpected chapters/topics response structure:', response);
      }
  ...
  async function createQuizAndStart() {
    ...
    try {
      ...
      if (!chaptersTopics || chaptersTopics.length === 0) {
        throw new Error('Chapters/Topics data is missing. Please reselect your options.');
      }
      ...
      if (chaptersTopicsArray.length === 0) {
        throw new Error('No chapters or topics selected. Please select some chapters or topics.');
      }
