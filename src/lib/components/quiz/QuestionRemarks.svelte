<script>
  ...
  $: if (remarks !== tempRemarks && document.activeElement !== textareaElement) {
    tempRemarks = remarks;
  }
...
<div class="mt-6 p-4 bg-slate-50 border border-slate-200 rounded-xl border-l-4 border-l-indigo-500">
  <div class="flex items-center gap-2 mb-3">
    ...
    {#if hasRemarks}
      <div class="flex items-center gap-1 text-xs bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded-full font-medium">
  ...
  {:else}
    ...
    <div class="bg-white border border-slate-200 rounded-lg overflow-hidden">
      ...
      <div class="flex justify-between items-center px-3 py-2 bg-slate-50 border-t border-slate-200">
        <div class="text-xs text-slate-500 {charactersLeft < 50 ? 'text-amber-600 font-medium' : ''}">
          {charactersLeft} characters remaining
        </div>
        <button
          on:click={saveRemarks}
          disabled={tempRemarks?.length > maxLength}
          class="px-4 py-1.5 text-xs font-medium bg-indigo-600 text-white rounded-md hover:bg-indigo-700 transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
        >
          Save
        </button>
      </div>
