<script>
	...
	export let nonClickableRoutes = ['masterdata','exams']; // Array of route segments that shouldn't be clickable (e.g., ['masterdata'])
	...
	function createBreadCrumbArray(route, params, searchParams) {
		...
		newRoute?.forEach((segment, index) => {
			...
			if (crumb) {
				...
				// Check if this route should be clickable
				const isClickable = !nonClickableRoutes.includes(segment);

				// Add the breadcrumb to the array
				breadCrumbsArr.push({ crumb, path: fullPath, isClickable });
			}
		});
	}
</script>
			...
			{#each breadCrumbsArr as breadCrumb, index}
				<li>
					<div class="flex items-end justify-center">
						{#if breadCrumb?.isClickable !== false && index !== breadCrumbsArr?.length - 1}
							<a
								href={breadCrumb?.path}
								class="text-xs lg:text-sm hover:text-gray-700 capitalize text-gray-90"
							>
								{breadCrumb?.crumb}
							</a>
						{:else}
							<span
								class="text-xs lg:text-sm capitalize {index === breadCrumbsArr?.length - 1
