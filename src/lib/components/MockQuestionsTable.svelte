<script>
  ...
  // Initialize original questions and initial exclusions
  $: if (questions.length > 0 && !initialized) {
    originalQuestions = [...questions];
    // Get initial excluded codes from store
    const storeExcluded = $apiPayloadStore.qtn_codes_to_exclude || [];
    excludedQuestions = [...storeExcluded];
    // Filter questions to exclude already excluded ones
    questions = questions.filter((q) => !excludedQuestions.includes(q.id));
    initialized = true;
  }
  ...
  $: filteredQuestions = questions.filter((question) => {
    ...
    const matchesMarks =
      !selectedMarks || question.marks?.toString() === selectedMarks;
  ...
  function restoreQuestion(questionId) {
    const questionToRestore = originalQuestions.find(
      (q) => q.id === questionId,
    );
    ...
    <div class="flex items-center justify-between">
      <div>
        <h4 class="text-sm font-medium text-yellow-800">
          {excludedQuestions.length} Question{excludedQuestions.length !== 1
            ? "s"
            : ""} Excluded
        </h4>
    ...
    <table class="min-w-full divide-y divide-gray-300">
      <thead class="bg-gray-50">
        <tr>
          ...
          <th
            scope="col"
            class="hidden sm:table-cell px-3 py-3.5 text-left text-xs font-medium uppercase tracking-wide text-gray-500"
          >
            Type
          </th>
          <th
            scope="col"
            class="hidden md:table-cell px-3 py-3.5 text-left text-xs font-medium uppercase tracking-wide text-gray-500"
          >
            Marks
          </th>
          <th
            scope="col"
            class="hidden lg:table-cell px-3 py-3.5 text-left text-xs font-medium uppercase tracking-wide text-gray-500"
          >
            Difficulty
          </th>
          <th
            scope="col"
            class="hidden xl:table-cell px-3 py-3.5 text-left text-xs font-medium uppercase tracking-wide text-gray-500"
          >
            Parent
          </th>
      ...
      <tbody class="divide-y divide-gray-200 bg-white">
        ...
        {#each paginatedQuestions as question (question.id)}
          <tr
            class="hover:bg-gray-50 {excludedQuestions.includes(question.id)
          ...
          >
            ...
            <td
              class="hidden md:table-cell px-3 py-4 text-sm text-gray-900 font-medium"
            >
              {question.marks}
            </td>
            ...
            <td
              class="hidden xl:table-cell px-3 py-4 text-sm text-gray-500 max-w-xs"
            >
            ...
            >
              <button
                ...
                on:click={() =>
                  excludedQuestions.includes(question.id)
      ...
      <div class="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
        ...
        <div>
          ...
          >
            ...
            <!-- Page Input -->
            <div
              class="flex items-center px-4 border-t border-b border-gray-300"
            >
