...
<div class="bg-gray-50 rounded-lg p-6 mb-6">
        ...
        >
          <span class="text-gray-500">
            {#if filters.roles.length === 0}
              Select roles...
            {:else if filters.roles.length === 1}
              {roleOptions.find(r => r.value === filters.roles[0])?.name}
            {:else}
              {filters.roles.length} roles selected
            {/if}
          </span>
            ...
            {#each roleOptions as role}
              <label class="flex items-center px-3 py-2 hover:bg-gray-50 cursor-pointer">
                <input
                  ...
                  on:change={(e) => handleMultiSelectChange('roles', role.value, e.target.checked)}
  ...
  {#if showTeacherFilters}
    <div class="mt-8 pt-6 border-t border-gray-200">
      <h4 class="text-sm font-medium text-gray-700 mb-4">Teacher-specific Filters</h4>
            ...
            >
              <span class="text-gray-500 text-sm">
                ...
                {:else if filters.blocks.length === 1}
                  {blockOptions.find(b => b.value === filters.blocks[0])?.name}
                {:else}
                ...
                {#each blockOptions as block}
                  <label class="flex items-center px-3 py-2 hover:bg-gray-50 cursor-pointer">
                    <input
                      ...
                      on:change={(e) => handleMultiSelectChange('blocks', block.value, e.target.checked)}
                      class="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                    />
            ...
            >
              <span class="text-gray-500 text-sm">
                ...
                {:else if filters.schools.length === 1}
                  {schoolOptions.find(s => s.value === filters.schools[0])?.name}
                {:else}
                ...
                {#each schoolOptions as school}
                  <label class="flex items-center px-3 py-2 hover:bg-gray-50 cursor-pointer">
                    <input
                      ...
                      on:change={(e) => handleMultiSelectChange('schools', school.value, e.target.checked)}
                      class="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                    />
            ...
            >
              <span class="text-gray-500 text-sm">
                ...
                {:else if filters.subjects.length === 1}
                  {subjectOptions.find(s => s.value === filters.subjects[0])?.name}
                {:else}
                ...
                {#each subjectOptions as subject}
                  <label class="flex items-center px-3 py-2 hover:bg-gray-50 cursor-pointer">
                    <input
                      ...
                      on:change={(e) => handleMultiSelectChange('subjects', subject.value, e.target.checked)}
                      class="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                    />
            ...
            >
              <span class="text-gray-500 text-sm">
                ...
                {:else if filters.classes.length === 1}
                  {classOptions.find(c => c.value === filters.classes[0])?.name}
                {:else}
                ...
                {#each classOptions as class_option}
                  <label class="flex items-center px-3 py-2 hover:bg-gray-50 cursor-pointer">
                    <input
                      ...
                      on:change={(e) => handleMultiSelectChange('classes', class_option.value, e.target.checked)}
                      class="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                    />
                    <span class="ml-3 text-sm text-gray-700">{class_option.name}</span>
                  </label>
                {/each}
              </div>
            {/if}
